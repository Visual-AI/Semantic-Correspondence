...
class SynthecticAffHomoTPSTransfo:
    ...
    def __init__(self, size_output_flow=(480, 640), random_t=0.25, random_s=0.5, random_alpha=np.pi / 12,
                 ...
                 transformation_types=None, parametrize_with_gaussian=False, proba_horizontal_flip=0.0, _device='cpu'):
        """
        ...
        if 'hom' in self.transformation_types:
            self.homo_grid_sample = HomographyGridGen(out_h=self.out_h, out_w=self.out_w, _device=self._device)
        if 'affine' in self.transformation_types:
            self.aff_grid_sample = AffineGridGen(out_h=self.out_h, out_w=self.out_w, _device=self._device)
        ...
        if 'tps' in self.transformation_types:
            self.tps_grid_sample = TpsGridGen(out_h=self.out_h, out_w=self.out_w, grid_size=tps_grid_size,
                                              reg_factor=tps_reg_factor, _device=self._device)
        if 'afftps' in self.transformation_types:
            self.tps_aff_grid_sample = ComposedGeometricTnf(tps_grid_size=tps_grid_size, tps_reg_factor=tps_reg_factor,
                                                            ...
                                                            padding_crop_factor=None, _device=self._device)
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self.parametrize_with_gaussian:
            if geometric_model == 'affine' or geometric_model == 'afftps':
                ...
                theta_aff = theta_aff.to(self._device)
            if geometric_model == 'hom':
                ...
                theta_hom = theta_hom.to(self._device)
            if geometric_model == 'tps':
                ...
                theta_tps = theta_tps.to(self._device)

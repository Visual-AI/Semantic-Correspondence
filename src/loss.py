...
class GaussianCrossEntropyLoss:     # Change the name to FeatMapLoss
    ...
    def __call__(self, src_featmaps, trg_featmaps, 
                 src_kps, trg_kps, 
                 src_imgsize, trg_imgsize, 
                 npts, softmax_temp, enable_l2_norm,
                 src_vis=None, trg_vis=None):
        ...
        for b in range(bz):
            ...
            # temp = softmax_temp[b:b+1].clone()
            temp = softmax_temp
            xy0 = src_kps[b:b+1].clone()
            xy1 = trg_kps[b:b+1].clone()
            npt = npts[b].clone()

            # if src_vis is not None and trg_vis is not None: # ap10k
            #     xy0_vis = src_vis[b]
            #     xy1_vis = trg_vis[b]
            #     vis = (xy0_vis * xy1_vis).bool()
            #     xy0_visible = xy0[0][vis,:2]
            #     xy1_visible = xy1[0][vis,:2]
            #     xy0 = xy0_visible.unsqueeze(0)
            #     xy1 = xy1_visible.unsqueeze(0)
            #     npt = torch.tensor(vis.sum().item()).to('cuda')
            ...
            xy0_corr = softmax_with_temperature(xy0_corr, temp, dim=-1).reshape(1, -1, h1, w1)

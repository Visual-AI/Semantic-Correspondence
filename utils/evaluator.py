...
class PCKEvaluator:
    ...
    def evaluate_feature_map(self, batch, softmax_temp=0.04, gaussian_suppression_sigma=7, enable_l2_norm=True):
        ...
        src_kps = scaling_coordinates(src_kps, (H1, W1), (h1, w1)) # scale src_kps to featmap size

        bilinear_matches = bilinear_get_matches(src_featmaps, trg_featmaps, src_kps, l2_norm=enable_l2_norm)
        bilinear_matches = scaling_coordinates(bilinear_matches, (h2, w2), (H2, W2)) # scale trg_featmaps to trg_img size
    ...
    def print_summarize_result(self):
        ...
        separator = "-" * 40
        header = " " * 16 + "".join([f"{alpha:<10}" for alpha in self.alpha])

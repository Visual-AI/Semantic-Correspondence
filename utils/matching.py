...
def l2_norm(feat, dim):
    """Perform L2 normalization on the input tensor along the specified dimension."""
...
def extract_feature(featmap, kps):
    ...
    if featmap.dtype != kps_.dtype:
        kps_ = kps_.to(featmap.dtype)
...
def bilinear_get_matches(src_featmaps, trg_featmaps, query, l2_norm=True):
    ...
    Find the top k matches by four corner bilinear interpolation
        ...
        top_k [Int]
    OUT:
        matches [torch.Tensor] (B x ... x 2): One more dimension (topk) than 'query'
    '''
...
def softargmax_get_matches(src_featmaps, trg_featmaps, query, softmax_temp, l2_norm=True):
    ...
    OUT:
        matches [torch.Tensor] (B x Nq x 1 x 2): One more dimension (1) than 'query'
    '''
